...
func handleTraderStart(w http.ResponseWriter, r *http.Request) {
	...
	go func(market string, closes *[]float64) {
		strategyWma(market, closes)
		for {
			select {
...
func strategyWma(market string, closes *[]float64) {
	fmt.Println("Tick", market, time.Now().String())
	temp := *closes
	*closes = temp[len(temp)-1000:] //take 1000 values
	candleStick, err := bittrex.GetLatestTick(market, "thirtyMin")
	if err != nil {
